...
import { completionStatusDisplay, completionStatusLabels, exerciseResultFor, hasRecordedWorkoutDetails, keySprintResult, loggedSessionCategoryLabels, readableDate, sorenessIndicator, workoutCategoryLabels, workoutToPlannedSnapshot } from '@/utils/training-history';
...
export default function HistoryDetailScreen() {
  ...
  const duplicate = async () => {
    ...
    try {
      ...
      error();
      Alert.alert('Could not duplicate workout', 'Please try again.');
    }
  };
  const remove = () => {
    if (!log) return;
    Alert.alert('Delete this training log?', 'This removes the saved workout from History and Progress on this device. This cannot be undone.', [
      ...
      { text: 'Delete log', style: 'destructive', onPress: async () => { warning(); await deleteTrainingLog(log.id); router.replace('/history'); } },
    ]);
  };

  if (!log) return <SafeAreaView style={styles.safe}><View style={styles.notFound}><MaterialIcons name="history" color={palette.muted} size={32} /><Text style={styles.notFoundTitle}>Training log not found</Text><Pressable onPress={() => { tap(); router.replace('/history'); }}><Text style={styles.link}>Back to History</Text></Pressable></View></SafeAreaView>;
