...
function FilterChip({ label, selected, onPress }: { label: string; selected: boolean; onPress: () => void }) {
  return <Pressable onPress={onPress} style={[styles.filterChip, selected && styles.filterChipActive]}><Text style={[styles.filterChipText, selected && styles.filterChipTextActive]}>{label}</Text></Pressable>;
}
...
export default function HistoryScreen() {
  ...
  const load = useCallback(() => { getTrainingLogs().then(setLogs); }, []);
  useFocusEffect(load);
  ...
  return <SafeAreaView style={styles.safe}><ScrollView contentContainerStyle={styles.page} keyboardShouldPersistTaps="handled">
    ...
    <ScreenTitle subtitle="Review what was planned, what happened, and how your body responded.">Training History</ScreenTitle>

    <View style={styles.actionRow}>
      <Pressable onPress={() => router.push('/log')} style={styles.manual}><MaterialIcons name="add" size={20} color={palette.accent} /><Text style={styles.manualText}>Log a session</Text></Pressable>
      <Pressable onPress={() => setFiltersOpen(current => !current)} style={[styles.filterButton, filtersOpen && styles.filterButtonActive]}><MaterialIcons name="tune" size={19} color={filtersOpen ? '#0B1000' : palette.accent} /><Text style={[styles.filterButtonText, filtersOpen && styles.filterButtonTextActive]}>Filters</Text></Pressable>
    </View>

    <View style={styles.searchWrap}><MaterialIcons name="search" size={20} color={palette.muted} /><TextInput value={filters.query} onChangeText={query => setFilters(current => ({ ...current, query }))} placeholder="Search workout names or notes" placeholderTextColor="#647382" style={styles.search} /></View>

    {filtersOpen && <Card style={styles.filtersCard}>
      <View style={styles.filterHead}><Text style={styles.filterTitle}>Filter history</Text>{hasFilters ? <Pressable onPress={() => setFilters(defaultHistoryFilters)}><Text style={styles.clearText}>Clear filters</Text></Pressable> : null}</View>
      <Text style={styles.filterLabel}>Date range · YYYY-MM-DD</Text>
      <View style={styles.dateRow}><TextInput value={filters.dateFrom} onChangeText={dateFrom => setFilters(current => ({ ...current, dateFrom }))} placeholder="From" placeholderTextColor="#647382" autoCapitalize="none" style={styles.dateInput} /><TextInput value={filters.dateTo} onChangeText={dateTo => setFilters(current => ({ ...current, dateTo }))} placeholder="To" placeholderTextColor="#647382" autoCapitalize="none" style={styles.dateInput} /></View>
