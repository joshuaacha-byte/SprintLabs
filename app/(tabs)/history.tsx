...
function HistoryRow({ log, onPress }: { log: TrainingLog; onPress: () => void }) {
  const palette = useTheme();
  const styles = useMemo(() => createStyles(palette), [palette]);
  const keyResult = keySprintResult(log);
  const summary = sessionSummary(log);
  const completed = log.completionStatus.startsWith('completed');
  const hasDetails = hasRecordedWorkoutDetails(log);
  return <Pressable onPress={onPress}><Card style={styles.logCard}>
    ...
    <View style={styles.metaRow}><Text style={styles.category}>{workoutCategoryLabels[log.plannedWorkout.trainingCategory]}</Text><Text style={[styles.completion, completed ? styles.completeText : styles.partialText]}>{completionStatusDisplay(log)}</Text></View>
    <Text style={styles.summaryLine} numberOfLines={2}>{hasDetails ? summary ?? 'Workout details not recorded.' : 'Workout details not recorded.'}</Text>
    <View style={styles.metrics}>{keyResult ? <Metric label="Key result" value={keyResult} tone="accent" /> : null}<Metric label="RPE" value={log.sessionRpe ? `${log.sessionRpe}/10` : '—'} secondary /></View>
  </Card></Pressable>;
}

function Metric({ label, value, tone, secondary }: { label: string; value: string; tone?: 'accent' | 'high' | 'moderate' | 'low'; secondary?: boolean }) {
  const palette = useTheme();
  const styles = useMemo(() => createStyles(palette), [palette]);
  return <View style={[styles.metric, secondary && styles.metricSecondary]}><Text style={styles.metricLabel}>{label}</Text><Text style={[styles.metricValue, secondary && styles.metricValueSecondary, tone === 'high' && { color: palette.red }, tone === 'moderate' && { color: palette.orange }, tone === 'accent' && { color: palette.accent }]} numberOfLines={1}>{value}</Text></View>;
}
