...
function Welcome() { return <View style={styles.welcome}><Text style={styles.welcomeTitle}>Build your speed profile.</Text><Text style={styles.subtitle}>A short conversation with Split, then SprintLab can organize the context behind your training.</Text></View>; }
function Question({ title, subtitle, children }: { title: string; subtitle?: string; children: React.ReactNode }) { return <View style={styles.question}><Text style={styles.questionTitle}>{title}</Text>{subtitle ? <Text style={styles.subtitle}>{subtitle}</Text> : null}{children}</View>; }
...
function SportProfileStep({ profile, update }: Props) {
  ...
  if (sport === 'baseball' || sport === 'softball') { const item = profile.baseballSoftballProfile ?? {}; return <Question title="Your baseball / softball profile" subtitle="Use the numbers players commonly know. All fields are optional."><PerformanceInput value={item.position ?? ''} onChangeText={position => update({ sportPosition: position, baseballSoftballProfile: { ...item, position } })} placeholder="Position" keyboardType="default" /><PerformanceInput value={String(item.homeToFirstTime ?? '')} onChangeText={value => update({ baseballSoftballProfile: { ...item, homeToFirstTime: setNum(value) } })} placeholder="Home-to-first in seconds (optional)" /><PerformanceInput value={String(item.sixtyYardDashTime ?? '')} onChangeText={value => update({ baseballSoftballProfile: { ...item, sixtyYardDashTime: setNum(value) } })} placeholder="60-yard dash in seconds (optional)" /></Question>; }
  if (sport === 'volleyball') { const item = profile.volleyballProfile ?? {}; return <Question title="Your volleyball profile" subtitle="Add familiar jump measurements if you know them. They are optional."><PerformanceInput value={item.position ?? ''} onChangeText={position => update({ sportPosition: position, volleyballProfile: { ...item, position } })} placeholder="Position" keyboardType="default" /><PerformanceInput value={String(item.verticalJump ?? '')} onChangeText={value => update({ volleyballProfile: { ...item, verticalJump: setNum(value) } })} placeholder="Standing vertical in inches (optional)" /><PerformanceInput value={String(item.approachJump ?? '')} onChangeText={value => update({ volleyballProfile: { ...item, approachJump: setNum(value) } })} placeholder="Approach jump in inches (optional)" /></Question>; }
...
function Chip({ label, selected, onPress }: { label: string; selected: boolean; onPress: () => void }) { return <Pressable accessibilityRole="button" accessibilityState={{ selected }} onPress={onPress} style={[styles.chip, selected && styles.chipSelected]}><Text style={[styles.chipText, selected && styles.chipTextSelected]}>{label}</Text></Pressable>; }
